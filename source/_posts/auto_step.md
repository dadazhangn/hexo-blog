...
4. App内我的->第三方接入，绑定微信 支付宝
    > 备注：同步微信运动请按照要求关注【华米科技】公众号
...
```
## 三、部署阿里云
  1. 注册[阿里云函数](https://common-buy.aliyun.com/?commodityCode=fc#/buy) 免费开通阿里云函数服务
    访问[服务列表](https://fcnext.console.aliyun.com/cn-shenzhen/services) 随便选择一个国内地区，点击【创建服务】。随便写名称，日志和链路追踪不建议开 
    ![](https://s2.loli.net/2024/02/27/JlfqRBDQjhNpwKL.png)
  2. 点击创建函数 -> 选择从零开始创建 -> 随便填个名称 -> 选择python3.6（其它都默认图中所示）
    ![](https://s2.loli.net/2024/02/27/cI8jLxnHt5CrfZB.png)
    ![](https://s2.loli.net/2024/02/27/Q4n5tDmF6xqeUpY.png)
  3. 往下拉配置触发器，选择定时触发器 -> 随便填个名称 -> 指定时间（如图中表示每天14点整开始运行，时间建议选择中午到下午）其它不用填，点击创建
    ![](https://s2.loli.net/2024/02/27/uO3rzJELKZ9ySjh.png)
  4. 接着把函数代码复制到index.py里面，填上手机号密码 -> 部署代码 -> 测试函数，可以看到执行成功了
    ![](https://s2.loli.net/2024/02/27/kRQPvc63FedrHh2.png) 
      > 备注：一定要先部署再测试

## 四、结语
  默认随机两到三万，刷的太多容易“失效”。步数太高的别参与微信排行榜，很容易被禁7天

  可以每天坐等到时间自动刷步，每日领取290g蚂蚁森林能量
